...
of packages have been developed on top of base graphics, such as grid, lattice,
and the popular `GGplot2` package. The current package builds on top of
`ggplot2`, but goes even a step further. We are aiming at an even higher level
of visualization, where the user has to care only about the "what" (data and
variable relations), and leaves the "how" (e.g., choice of the type of diagram)
to the software.
...
it is often a good visual aid to show an average line. There are only three
different species in the data set, so we won't overcrowd the graph by using
different colors. So, all in all we might write the following lines:
...
As we see, `ggplot2` makes it quite easy to create a plot. However, we still have to think about representation. What if we could just concentrate on the relation we want to visualize? The following would be the equivalent in `plotluck`:
...
Admittedly in this simple example there is not a lot of reduction in typed lines, however, often some more thought and preprocessing is necessary. For mixed data types, the best plot type is not always as obvious. You might also want to quickly visualize two variables without first inspecting their type - especially if you make a trellis plot of all of them (see below for an example).

After looking at a graph for the first time, you might notice that outliers make it hard to see most of the data, so you plot it again using a logarithmic axis transform. If a factor has a large number of levels, you might want to sort them first by the value of the dependent variable, or cluster many infrequent levels into a single default value. You want to accurately account for observation weights and missing values.

_Plotluck_ is a tool for exploratory data visualization in R that automates such steps. Similarly as going from base graphics to higher level functions, it simplifies and speeds up the process of data visualization; equally similar, this comes at the price of a reduced flexibility in specializing and "tweaking" individual plots.

## A Longer Example

Let us eplore Hans Rosling's famous _gapminder_ data. At first, we might just want to know what variables it contains and how they are distributed. In plotluck, the dot symbols stands for "all variables
in the data set":
...
In this grid view, each distribution is represented very minimally like a thumbnail picture, but we get a general sense of the data. There are 2 categorical and 4 numeric variables; the amount of data is uniform for a range of years; one continent has very few, another one very many rows; `pop` and `gdpPercap` have very skewed distributions, so a log-transform has been applied. 
...
As can be seen from the printout, conditional entropies are estimated for ordering the plot (lower values indicate stronger informational value). From the overview, we see a clear correlation between `lifeExp` and `gdpPercap`, as well as `year` and continent. Lets zoom in on the latter:
...
It is also possible (and can be fun) to produce random visualization over a dataset:
...
* The type of these variables; mainly, it distinguishes between numeric, ordered (ordinal data), and 
  ...
  for visualization it sometimes makes sense to treat them similarly as numeric types, e.g., for sorting other dependent factors.
* Additionally but less importantly, a few heuristics depend on instance counts.   
  E.g., for small data sets, simple scatter plots can reveal more details than density diagrams or histograms; spine plots
  get confusing with too many factor levels; and if there is only a single data point per factor 
  combination, we would rather see a bar plot than a degenerated box plot.
...
The relation between two factor variables can be depicted best by spine (a.k.a.,
mosaic) plots, unless they have too many levels. Otherwise, the fallback is a heat map.

For a mixed-type (factor/numeric) pair of variables, violin plots are generated.
However, if the resulting graph would contain too many plots in a row, the
...
Conditional variables are represented by either trying to
fit into the same graph using coloring or by facetting. Numeric vectors are 
...
added by default: Density plots and histograms come with an overlaid vertical
...
keep these in mind. The color value represents a 'central tendency' of the data 
instances closest to the grid point, but the variation within this bin cannot be
shown. If the color represents a numeric variable, the central value is computed
as the median; if it is an ordered factor, it is the factor closest to the 
median, when treating the levels as integers; and for unordered factors, it is 
the mode (most frequent value). Especially in the last case, this value might 
not be very meaningful if the class distribution is close to uniform.
...
* Correct handling and visualization of instance weights. All calculations correctly take weights into account when supplied.
